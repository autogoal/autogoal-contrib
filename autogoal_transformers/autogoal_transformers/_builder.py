...
from autogoal_transformers._utils import (
    download_text_classification_models_info,
    to_camel_case,
)
import black
import enlighten
...
class PetrainedTextClassifier(TransformersWrapper):
    ...
    def _train(self, X, y=None):
        if not self._check_input_compatibility(X, y):
            raise Exception(f"Input is not compatible with target pretrained model ({self.name})")
        return y
...
def build_transformers_wrappers(download_file_path = None):
    imports = _load_text_classification_models_info(download_file_path)
    ...
    with open(path, "w") as fp:
        fp.write(
            ...
            from autogoal_transformers._builder import TextClassificationModel,
            from autogoal.kb import *
            """
            )
        )
...
def _load_text_classification_models_info(
    file_path=None, max_amount=1000
):
    ...
    if not os.path.exists(file_path):
        download_text_classification_models_info(file_path, max_amount)
...
def _write_class(item, fp):
    ...
    base_class = PetrainedTextClassifier.__name__
...
if __name__ == "__main__":
    build_transformers_wrappers(download_file_path="zero_shot_classification_models_info.json")
